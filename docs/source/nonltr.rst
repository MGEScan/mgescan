...
MGEScan-nonLTR is a program to identify non-long terminal repeat (non-LTR) retrotransposons in genomic sequences. A few options are available in the Galaxy workflow system to configure settings of the program, e.g. hmmsearch of protein sequence database with a profile hidden Markov model (HMM). 
